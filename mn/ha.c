...
struct _mn
{
  ...
  int in_nat[NIF];            //0 not in nat, 1 in nat
  struct in6_addr coav6; //mn's v6 coa
  struct in_addr coav4[NIF]; //mn's v4 coa
  struct in_addr natv4[NIF]; //mn's v4 nat server address
  int last_natv4_t[NIF];      //last time when probe is received
  uint16_t nat_port[NIF];     //mn's nat port
  ...
  for(;keepworking;){
	...
	if(n>0&&nat_probe.checksum==(nat_probe.ifindex^nat_probe.seqno^nat_probe.port)){
	  ...
	  if(imn==MAX_MN_NUM||icoa>=NIF){
	    fprintf(stderr,"nat probe: Invalid MN's HOA(%x:%x:%x:%x:%x:%x:%x:%x)!\n",
	...
	if(ifindex>0){
	  ...
	  if(imn==MAX_MN_NUM){
	    ...
	    if(IN6_IS_ADDR_V4MAPPED(&coa)){
	      ...
	      int icoa=0;
	      for(;memcmp(mns[imn].coav4+icoa,&coav4,sizeof(struct in_addr))&&icoa<NIF;
		  ++icoa);
		//fprintf(stderr,"comparing %d.%d.%d.%d with %d.%d.%d.%d\n",
		//NIP4ADDR(mns[imn].coav4+icoa),
		//NIP4ADDR(&coav4));
	      if(icoa==NIF){
		fprintf(stderr,"ERROR! BU's v4 COA not found in database.\n");
	      }
	      else {
	      	fprintf(stderr,"imn = %d, icoa = %d\n",imn,icoa);
		...
		mns[imn].active=icoa;
		fprintf(stderr,"sending ba, no coa\n");
...
int main(int argc,char * argv[])
{
  ...
  inet_pton(AF_INET6,"3ffe:501:ffff:100:215:58ff:fe2a:82c",&hav6);
  inet_pton(AF_INET,"10.21.5.53",&hav4);
  ha_init();
